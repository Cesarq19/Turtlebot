...
void TurtleBot3::init_dynamixel_sdk_wrapper(const std::string & usb_port)
{
  ...
  this->declare_parameter<uint8_t>("u2d2.id");
  this->declare_parameter<int>("u2d2.baud_rate");
  this->declare_parameter<float>("u2d2.protocol_version");

  this->get_parameter_or<uint8_t>("u2d2.id", opencr.id, 200);
  this->get_parameter_or<int>("u2d2.baud_rate", opencr.baud_rate, 1000000);
  this->get_parameter_or<float>("u2d2.protocol_version", opencr.protocol_version, 2.0f);
