...
#include "turtlebot3_mobility/sensors/joint_state.hpp"

using ramel::tb3Custom::sensors::JointState;
  ...
  std::shared_ptr<DynamixelSDKWrapper> & dxl_sdk_wrapper)
{
  ...
  dxl_sdk_wrapper->read_data_to_motors(0, extern_control_table.present_position.addr, extern_control_table.present_position.length, &present_position_left);
  dxl_sdk_wrapper->read_data_to_motors(1, extern_control_table.present_position.addr, extern_control_table.present_position.length, &present_position_right);

  dxl_sdk_wrapper->read_data_to_motors(0, extern_control_table.present_velocity.addr, extern_control_table.present_velocity.length, &present_velocity_left);
  dxl_sdk_wrapper->read_data_to_motors(1, extern_control_table.present_velocity.addr, extern_control_table.present_velocity.length, &present_velocity_right);
